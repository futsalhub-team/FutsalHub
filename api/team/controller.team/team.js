const { Team, TeamMember, TeamRecruitment } = require('../../../models');
...
const createRecruitment = async (req, res) => {
    ...
    try {
        ...
        const recruitment = await TeamRecruitment.create({
            userId: userId,
            title: title,
            content: content
        });

        return response(res, 200, recruitment);
    } catch (err) {
        console.error(err);
        return response(res, 500, '모집글 생성 실패');
    }
}

const getRecruitmentList = async (req, res) => {
    try {
        const recruitmentList = await TeamRecruitment.findAll();
        return response(res, 200, recruitmentList);
    } catch (err) {
        console.error(err);
        return response(res, 500, '모집글 리스트 조회 실패');
    }
}


module.exports = {
    createTeam, addMember, getTeamList, getTeamDetail, createRecruitment, getRecruitmentList
};